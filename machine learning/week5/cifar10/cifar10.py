...
def inference(images):
  ...
  # # TODO: pool3 What strides should we use?
  ...
  #
  # # pool6
  # pool6 = tf.nn.max_pool(conv5, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
  ...
  # # pool8
  # pool8 = tf.nn.max_pool(conv7, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
  ...
  #     softmax_linear = tf.nn.softmax(tf.matmul(local9, weights) + biases, name=scope.name)
  ...
  #
  #   output = tf.nn.softmax(softmax_linear)
...
def accuracy(logits,labels):
    ...
    num_rows = labels.get_shape().as_list()
    return tf.reduce_sum(tf.cast(top_k_op, tf.float32))/num_rows
